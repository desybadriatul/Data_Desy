"""
Task 2 workflow: Industry Trend Report.
...
Prinsip:
- KPI / sentiment / channel / author / content SELALU memakai full canonical data.
...
WORKFLOW_VERSION = "industry_trend_report_workflow_v1"
...
class IndustryTrendWorkflowError(RuntimeError):
    """Raised when the Industry Trend workflow cannot proceed safely."""
    ...
    *, project_name: str, start_date: str, end_date: str,
    ...
    try:
        ...
        payload.update({
            ...
            "fallback_instruction": (
                ...
                "Thematic & Topic Trends akan muncul sebagai limitation."
            ),
        })
        return payload
    ...
    payload.update({
        ...
        "assistant_next_steps": [
            ...
            "Topic harus report-level (tema industri), bukan entity "
            ...
            "Lalu panggil create_industry_trend_report_workflow lagi dengan "
        ...
        "user_visible_progress_message": (
            "Saya akan membuat topic taxonomy industri otomatis dari sample "
    ...
    *, project_name: str, taxonomy_version: str, start_date: str, end_date: str,
    ...
    payload.update({
        ...
        "assistant_next_steps": [
            ...
            "Pakai not_relevant untuk post di luar scope industri.",
        ...
        "user_visible_progress_message": (
            ...
            "industri, lalu menampilkan preview data sebelum PPT."
        ),
    })
    return payload
...
#  API publik
# ----------------------------------------------------------------------
def create_industry_trend_report_workflow(
    *,
    project_name: str,
    start_date: str,
    ...
    """Satu perintah untuk membuat Industry Trend Report end-to-end."""
    ...
    if not project_name or not start_date:
        raise IndustryTrendWorkflowError("project_name dan start_date wajib diisi.")
    ...
    if topic_enabled and eligible > 0:
        if not taxonomy_version or status_before.get("status") == "NEEDS_TAXONOMY":
            return _taxonomy_seed_payload(
                ...
                suggested_taxonomy_version=f"{_slug(project_name)}_industry_topic_v1",
    ...
    if not report_input_id:
        raise IndustryTrendWorkflowError(
            "Task 1 tidak mengembalikan report_input_id; tidak bisa lanjut."
        )

    preview = build_industry_trend_report_data_preview(
    ...
    if not confirm_pptx and _needs_ppt_confirmation(preview, ask_before_pptx):
        ...
        payload.update({
            ...
            "confirmation_question": (
                f"Data preview Industry Trend {project_name} ({period_label}) "
    ...
    package = build_industry_trend_report_package(
        report_input_id, audience_context=audience, audience_pov=report_pov,
    )
    return {
        "success": True,
