...
## 0. Pengingat jalur — baca dulu

File ini di-return oleh `get_report_guide()`, jadi hampir selalu terbaca lebih
dulu. Sebelum memakai isinya, pastikan jalurnya sudah benar.

> **Pertanyaannya satu: user MENYEBUT tipe report, atau tidak?**

**Menyebut** ("daily report", "MMR", "competitive analysis", "BCE",
"industry trend", "SFIR") → **JALUR 1, bottom-up.**
Serahkan ke `create_*_report_workflow`. Alurnya pakem:
workflow → Task 1 preview → konfirmasi user → Task 2 → PPT.
File ini **tidak dipakai** untuk merakit slide di Jalur 1.

**Tidak menyebut** ("ada topik apa minggu ini?", "apa yang terjadi pada brand
A?", "cek data brand A dan B", atau sekadar "bikin laporan"/"saya mau PPT")
→ **JALUR 2, top-down.** Di sinilah file ini berlaku penuh.

Urutan Jalur 2:

```text
Intent Confirmation 8 poin  -> tunggu persetujuan user
  -> validate_metric_readiness() + data_health()
  -> diagnosis: scan_anomalies(), timeline(), get_posts(), dst
  -> laporkan temuan ke user
  -> simpulkan jenis masalahnya, konfirmasi singkat
  -> rakit deck: file ini (cerita) + perpustakaan_resep_slide.md (visual)
```

Di Jalur 2, **dilarang** memanggil `create_*_report_workflow`,
`prepare_report_input`, atau `build_*_ppt_package`.

**Ragu?** Berarti user tidak menyebut. **Jalur 2.**

Detail lengkap ada di `SKILL.md` Bagian 0.
